...
module.exports = {
  forbidden: [
    ...
    // 2) apps끼리 직접 참조 금지 (api-e2e는 예외로 둘 수도)
    {
      name: 'no-web-to-api',
      severity: 'error',
      from: { path: '^apps/web/' },
    ...
    // 3) domain은 “순수”하게 (인프라/프론트/백엔드 레이어에 의존 금지)
    {
      name: 'domain-must-be-pure',
      comment: 'domain must not depend on infra, ui, or backend implementation packages',
      severity: 'error',
      from: { path: '^packages/domain/' },
      to: { path: '^packages/(database|ui|backend-|contract)/' },
    },

    // 4) web은 백엔드 구현 패키지에 의존 금지 (프론트는 ui/common/contract 정도만)
    {
      name: 'web-no-backend-impl',
      severity: 'error',
      from: { path: '^apps/web/' },
      to: { path: '^packages/(backend-|database)/' },
    },

    // 5) backend-*는 ui에 의존 금지
    {
      name: 'backend-no-ui',
      severity: 'error',
    ...
    // 6) config 패키지는 앱 코드에 의존 금지
    {
      name: 'configs-no-apps',
      severity: 'error',
      from: { path: '^packages/(eslint-config|jest-config|typescript-config)/' },
      to: { path: '^apps/' },
    },
  ],
  options: {
...
// generated: dependency-cruiser@17.3.2 on 2025-12-12T13:52:50.495Z
